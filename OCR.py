...
class OCR_Boxes:
    ...
    def sendPost(self, licensePlate):
        API_ENDPOINT= 'http://localhost:8080/camera/licencePlate/something'
        
        #datas = {'licensePlate':licensePlate}

        #header = {'content-type': 'application/json'}
        licensePlate=re.sub('[!@#$]', '', licensePlate)
        
        licensePlate=re.sub(r'[^\w]', '', licensePlate)
        r = requests.post(url = API_ENDPOINT, data = licensePlate)
        # headers = header,
    ...
    def put_ocr_boxes(self, boxes, frame, height, crop_width=0, crop_height=0, view_mode=1):
        """
                       ...
                       # y += crop_height
                        ...
                        if int(conf) > conf_thresh:
                           # cv2.rectangle(frame, (x, y), (w + x, h + y), color, thickness=1)
                            text = text + ' ' + word # for continous sentence instead of new lines of a sentence detected
...
#def databaseCon():
 #   conn = psycopg2.connect("dbname=jdbc:h2:~/data/demo user=sa password='' host=localhost port=5435")
  #  cur = conn.cursor()
   # return cur
...
def ocr_stream(crop: list[int, int], source: int = 0, view_mode: int = 1, language=None):
    ...
    while True:
        ...
        frame, text = ocrbox.put_ocr_boxes(boxes=ocr.boxes, frame=frame, height=img_hi,
